...
- Added Scan Bin mode with a phone-first scan tray and scanner focus action.
...
- Browser QA passed with live shared state `frostbite-flow-live`, `714` bins, Scan Bin tray on desktop and mobile, QR target `#55-1-02`, mobile hash target `#10-1-01`, zero blocked requests, and zero browser errors.
...
- Browser QA confirmed no Shopify requests occurred and no visible Shopify workflow copy is present.
...
- Shopify remains read-only behind the scenes and is not exposed as an operator workflow.
